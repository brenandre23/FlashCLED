...
def enforce_data_availability_consistency(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ensures availability flags are 0 if their dependent features are missing (NaN).
    """
    logger.info("Enforcing Data Availability Consistency...")

    for flag_col, feature_cols in FLAG_TO_FEATURES_MAP.items():
        ...
        # If any dependent feature is NaN, the data is effectively missing
        mask_missing_data = df[valid_feats].isna().any(axis=1)
        mask_inconsistent = (df[flag_col] == 1) & mask_missing_data

        count = mask_inconsistent.sum()
        if count > 0:
            logger.info(f"  Correction: Flipping {flag_col} to 0 for {count:,} rows due to missing data.")
            df.loc[mask_inconsistent, flag_col] = 0
...
# --- FLAG TO FEATURES MAPPING ---
# Used to enforce consistency: if these features are NaN, the flag MUST be 0.
FLAG_TO_FEATURES_MAP = {
    # General Coverage
    'econ_data_available': ['gold_price_usd_lag1', 'oil_price_usd_lag1', 'sp500_index_lag1', 'eur_usd_rate_lag1'],
    'viirs_data_available': ['nightlights_intensity'],
    'iom_data_available': ['iom_displacement_count_lag1'],
    'landcover_data_available': ['landcover_grass', 'landcover_trees', 'landcover_crops', 'landcover_built'],

    # Food Security (General + Specific High-Importance Drivers)
    'food_data_available': ['food_price_index'],
    'price_rice_available': ['price_rice', 'price_rice_shock'],
    'price_oil_available': ['price_oil', 'price_oil_shock'],
    'price_maize_available': ['price_maize', 'price_maize_shock'],
}
...
def process_food_security(engine, spine, social_specs, feat_cfg):
    ...
    """
    impute_cfg = feat_cfg
    # --- FIX: Extract step_days from config ---
    temporal_cfg = feat_cfg.get('temporal', {}) if isinstance(feat_cfg, dict) else {}
    step_days = temporal_cfg.get('step_days', 14)  # Default to 14 if missing
    # ------------------------------------------
    ...
    if has_locs and has_prices:
        ...
        if not locs.empty and not prices.empty:
            ...
            if not prices.empty:
                ...
                # --- NEW: Generate Targeted Availability Flags (pre-imputation) ---
                for commodity in ['price_rice', 'price_oil', 'price_maize']:
                    if commodity in spine.columns:
                        spine[f'{commodity}_available'] = spine[commodity].notna().astype(int)
                    else:
                        spine[f'{commodity}_available'] = 0
    ...
    if iom_spec:
        ...
        if inspector.has_table("iom_displacement_h3", schema=SCHEMA):
            ...
            spine['iom_displacement_sum'] = (
                spine.groupby('h3_index')['iom_displacement_sum'].ffill()
            )
...
# PHASE 3: ENVIRONMENTAL DATA (Config-Driven Anomalies + Trends)
...
def process_environment(engine, spine, env_specs, feat_cfg):
    ...
    # Create any synthesized/missing raws as NaN
    for raw in synthesized:
        spine[raw] = np.nan

    # DynamicWorld availability flag (data begins 2017-03-07 for Sentinel-2B/DW)
    if any(spec.get('source') == 'DynamicWorld' for spec in env_specs):
        spine['landcover_avail'] = (spine['date'] >= pd.Timestamp("2017-03-07")).astype(int)
    
    # 3. Calculate Climatology (Baselines) - Only for features needing 'anomaly'
    ...
    if anomaly_specs:
        ...
        for col in anomaly_raws:
            subset = env_df
            # Filter NTL (Nightlights) to avoid early noise if needed
            if 'ntl' in col:
                subset = env_df[env_df['year'] >= 2012]
                
            stats = subset.groupby(['h3_index', 'epoch'])[col].agg(['mean']).reset_index()
            stats.columns = ['h3_index', 'epoch', f"{col}_mean"]
            
            spine = spine.merge(stats, on=['h3_index', 'epoch'], how='left')
    
    # 4. Apply Transformations (Anomaly, Trend, Delta)
    for spec in env_specs:
        ...
        # --- A. Anomaly Transformation ---
        if 'anomaly' in trans:
            anom_col = f"{raw}_anom"
            spine[anom_col] = spine[raw] - spine[f"{raw}_mean"]
            spine[out] = spine[anom_col] # Output the anomaly as the primary feature
            
            # [NEW] Vegetation Trend (NDVI/EVI Only)
            # 2-Year Rolling Mean of Anomaly -> "Is the land structurally failing?"
            # We strictly limit this to NDVI to avoid redundancy.
            if 'ndvi' in raw.lower() or 'evi' in raw.lower():
                trend_col = f"{raw}_trend_2yr"
                spine[trend_col] = spine.groupby('h3_index')[anom_col].transform(
                    lambda x: x.rolling(window=52, min_periods=26).mean()
                ).fillna(0.0)
                logger.info(f"  > Generated Degradation Trend: {trend_col}")
            
        elif 'lag' in trans:
            spine[out] = spine.groupby('h3_index')[raw].shift(1)
             
        else:
            # Default passthrough (e.g., for Land Cover raw values)
            spine[out] = spine[raw]

        # --- B. [NEW] Land Cover Structural Change (Dynamic World) ---
        # 2-Year Absolute Delta -> "Did the city expand? Did the forest disappear?"
        # Applies to ANY variable where the OUTPUT name contains 'landcover' or source is DynamicWorld
        if 'landcover' in out.lower() or source == 'DynamicWorld':
            change_col = f"{out}_change_2yr"
            
            # 52 steps = ~2 years (14-day pipeline step)
            spine[change_col] = spine.groupby('h3_index')[out].transform(
                lambda x: x.diff(periods=52)
            )
            # Fill start of history with 0 (Assume stability)
            spine[change_col] = spine[change_col].fillna(0.0)
            logger.info(f"  > Generated Structural Change Delta: {change_col}")

    # Cleanup intermediate columns
    drop_cols = [c for c in spine.columns if c.endswith('_mean') or c.endswith('_anom')]
    protected = [
        'water_local_mean', 'epr_status_mean', 'elevation_mean', 'slope_mean',
        'gdelt_goldstein_mean', 'chirps_precip_mean', 'era5_temp_mean',
        'ndvi_max'
    ]
    # Keep output columns defined in specs plus the new auto-generated ones
    outputs = [s.get('output_col') for s in env_specs]
    
    # We must explicitly protect the new dynamic columns from being dropped
    new_features = [c for c in spine.columns if c.endswith('_trend_2yr') or c.endswith('_change_2yr')]
    
    drop_cols = [c for c in drop_cols if c not in protected and c not in outputs and c not in new_features]
...
def process_conflict(engine, spine, conflict_specs, features_config):
    ...
    # CrisisWatch aggregate score (sum of spatial_confidence)

    # ---------------------------------------------------------

    if insp.has_table("features_crisiswatch", schema=SCHEMA):

        logger.info("  Loading CrisisWatch aggregate score (cw_score_local)...")

        cw_agg_query = f"""

            SELECT h3_index, date, SUM(spatial_confidence) AS cw_score_local

            FROM {SCHEMA}.features_crisiswatch

            GROUP BY h3_index, date

        """

        cw_agg = pd.read_sql(cw_agg_query, engine)

        if not cw_agg.empty:

            cw_agg['date'] = pd.to_datetime(cw_agg['date'])

            cw_agg['h3_index'] = cw_agg['h3_index'].astype('int64')

            spine = safe_merge(spine, cw_agg, on=['h3_index', 'date'], how='left')

        if 'cw_score_local' not in spine.columns:

            spine['cw_score_local'] = 0

        spine['cw_score_local'] = spine['cw_score_local'].fillna(0)

        logger.info(f"  ✓ Loaded {len(cw_agg):,} aggregated CrisisWatch records.")

    else:

        logger.warning("  features_crisiswatch table not found; cw_score_local will be 0.")
    ...
    # FIX: GDELT Transformations (90d Decay + Renaming)

    # ---------------------------------------------------------

    if 'gdelt_event_count_lag1' in spine.columns:

        steps_90 = (

            features_config.get('temporal', {}).get('decays', {})
        ...
        alpha_90 = 1 - np.exp(-np.log(2) / float(steps_90))

        spine['gdelt_event_count_lag1_decay_90d'] = (

            spine.groupby('h3_index')['gdelt_event_count_lag1']

            .apply(lambda s: s.ewm(alpha=alpha_90, adjust=False).mean())
    ...
    if 'gdelt_event_count_lag1_decay_30d' in spine.columns:

        spine['gdelt_decay_30d'] = spine['gdelt_event_count_lag1_decay_30d']



    if 'gdelt_avg_tone_lag1_decay_30d' in spine.columns:

        spine['gdelt_avg_tone_decay_30d'] = spine['gdelt_avg_tone_lag1_decay_30d']



    if 'gdelt_event_count_lag1_decay_90d' in spine.columns:

        spine['events_3m_lag'] = spine['gdelt_event_count_lag1_decay_90d']
...
def process_crisiswatch_nlp(engine, spine, nlp_specs):

    """Process NLP signals (currently CrisisWatch per-topic pivots only)."""
    ...
    for spec in nlp_specs:
        ...
        # Only handle CrisisWatch per-topic pivots

        if source == "NLP_CrisisWatch" and transformation == "pivot_topic":
            ...
            value_col = params.get('value_column', 'spatial_confidence')

            if topic_id is None:

                logger.warning("  pivot_topic missing 'topic_id'; skipping.")
            ...
            cw_raw = pd.read_sql(
                ...
                f"SELECT h3_index, date, cw_topic_id, {value_col} FROM {SCHEMA}.features_crisiswatch",
            ...
            cw_raw = cw_raw[cw_raw['cw_topic_id'] == topic_id]

            if cw_raw.empty:

                logger.info(f"  No CrisisWatch rows for topic_id={topic_id}; skipping.")
            ...
            agg_df = (
                ...
                .groupby(['h3_index', 'spine_date'], observed=True)[value_col]

                .sum()

                .reset_index()

                .rename(columns={'spine_date': 'date', value_col: output_col})
            ...
            spine = spine.merge(agg_df, on=['h3_index', 'date'], how='left')

            spine[output_col] = spine[output_col].fillna(0)

            logger.info(f"  ✓ Merged CrisisWatch topic_id={topic_id} into {output_col}.")
...
def run():
    ...
    try:
        ...
        # --- FIX: Generate Targeted Availability Flags (Pre-Imputation) ---
        for commodity in ['price_rice', 'price_oil', 'price_maize']:
            if commodity in spine.columns:
                spine[f'{commodity}_available'] = spine[commodity].notna().astype(int)
            else:
        ...
        for col in price_cols:
            ...
            # Forward-fill only after first observation
        ...
        # Single availability flag (Corrected Indentation)
        ...
        admin_cols = [c for c in spine.columns if c.startswith('admin')]
        if 'level_0' in spine.columns:
            admin_cols.append('level_0')
        if admin_cols:
            logger.info(f"Dropping metadata columns: {admin_cols}")
            spine.drop(columns=admin_cols, inplace=True)
        ...
        # ---------------------------------------------------------
        # Enforce availability consistency before final fills
        # ---------------------------------------------------------
        spine = enforce_data_availability_consistency(spine)
        ...
        # Final safety fill for DB write
        spine = spine.fillna(0)
        ...
        integer_cols = [
            ...
            "fatalities", "time_since_last_fatal_event", "pop_count", 
            "iom_displacement_sum", "is_dry_season", "iom_data_available",

            # NEW: Targeted Food Security Flags (Must be Integers)
            "price_rice_available", 
            "price_oil_available", 
            "price_maize_available"
        ]
        # Protect continuous trend features (e.g., ndvi_trend_2yr) from integer coercion
        integer_cols = [c for c in integer_cols if not c.endswith("_trend_2yr")]
